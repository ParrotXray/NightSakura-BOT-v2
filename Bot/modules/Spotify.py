...
class SpotifyAddons():
    ...
    async def sanitize(self, name, replace_with=""):
        """
    ...
    async def fetch_tracks(self, item_type, item_id):
        """
    ...
    async def parse_spotify_url(self, url):
        ...
        if url.startswith("spotify:"):
            logging.getLogger.error("Spotify URI was provided instead of a playlist/album/track URL.")
            sys.exit(1)
    ...
    async def get_item_name(self, item_type, item_id):
        ...
        return await self.sanitize(name)


    async def validate_spotify_urls(self, urls):
        """
        ...
        valid_urls = []
        for url in urls:
            item_type, item_id = await self.parse_spotify_url(url)
            logging.getLogger.debug("Got item_type %s and item_id %s", item_type, item_id)
            if item_type not in ["album", "track", "playlist"]:
                logging.getLogger.info("Only albums/tracks/playlists are supported and not %s", item_type)
                continue
            if item_id is None:
                logging.getLogger.info("Couln't get valid item_id for %s", url)
            else:
